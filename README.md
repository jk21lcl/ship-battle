...
- `幽灵船`(Specter Ship): 15点生命，配备`幽灵弹`、`幽灵弹`
  ...
  - 技能3: `幽灵引爆`(Specter Explode): 对指定敌方目标造成(`幽灵`层数+2)的伤害，并消耗所有`幽灵`层数。冷却: 3回合
  - 技能4: `超级幽灵引爆`(Super Specter Explode): 限定技，对指定敌方目标造成(`幽灵`层数×3+5)的伤害，对相邻序号目标造成(`幽灵`层数×10%)比例的溅射伤害，同时消耗所有`幽灵`层数。
