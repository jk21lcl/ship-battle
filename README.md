...
### 沙盒人机模式 (sandbox + computer)

可以自由设置两方的初始配置，进行对战，其中玩家2为电脑。

### 随机人机模式 (random + computer)

可以设定初始双方的金钱数量，其中先手方为后手方的65%，同时可以指定禁用一些船只。
系统按照金钱和禁用列表随机生成两方的船只，保证剩余钱数无法再买任何一艘船，之后双方进行对战，其中玩家2为电脑。